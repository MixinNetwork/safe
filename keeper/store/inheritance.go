...
var inheritanceLockCols = []string{"lock_id", "request_hash", "hash", "holder", "address", "chain", "duration", "state", "created_at", "updated_at"}
...
func (s *SQLite3Store) processInheritanceLockOperation(ctx context.Context, tx *sql.Tx, lock *InheritanceLock) error {
	...
	if !existed {
		vals := []any{lock.LockId, lock.RequestHash, lock.Hash, lock.Holder, lock.Address, lock.Chain, lock.Duration, lock.State, lock.CreatedAt, lock.UpdatedAt}
	...
	err = s.execOne(ctx, tx, "UPDATE inheritance_locks SET request_hash=?, hash=?, duration=?, state=?, updated_at=? WHERE lock_id=?",
		lock.RequestHash, lock.Hash, lock.Duration, lock.State, lock.UpdatedAt, lock.LockId)
	if err != nil {
...
func (s *SQLite3Store) readInheritanceLock(ctx context.Context, tx *sql.Tx, id string) (*InheritanceLock, error) {
	...
	err := row.Scan(&lock.LockId, &lock.RequestHash, &lock.Hash, &lock.Holder, &lock.Address, &lock.Chain, &lock.Duration, &lock.State, &lock.CreatedAt, &lock.UpdatedAt)
...
func (s *SQLite3Store) ListUnfailedInheritanceLocksByHolder(ctx context.Context, holder string) ([]*InheritanceLock, error) {
	...
	for rows.Next() {
		var lock InheritanceLock
		err = rows.Scan(&lock.LockId, &lock.RequestHash, &lock.Hash, &lock.Holder, &lock.Address, &lock.Chain, &lock.Duration, &lock.State, &lock.CreatedAt, &lock.UpdatedAt)
		if err != nil {
			return nil, err
		}
		ls = append(ls, &lock)
