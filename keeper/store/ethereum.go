...
func (s *SQLite3Store) UpdateEthereumBalanceFromRequest(ctx context.Context, safe *Safe, txHash string, index int64, amount *big.Int, req *common.Request, assetId, safeAssetId, assetAddress, sender string) error {
	s.mutex.Lock()
	...
	if err != nil {
		...
		vals := []any{safe.Address, assetId, assetAddress, safeAssetId, amount.String(), txHash, time.Now().UTC(), false}
		err = s.execOne(ctx, tx, buildInsertionSQL("ethereum_balances", cols), vals...)
		if err != nil {
			return fmt.Errorf("INSERT ethereum_balances %v", err)
		}
	} else {
		err = s.execOne(ctx, tx, "UPDATE ethereum_balances SET balance=?, latest_tx_hash=?, updated_at=? WHERE address=? AND asset_id=? AND safe_asset_id=?", amount.String(), txHash, time.Now().UTC(), safe.Address, assetId, safeAssetId)
		if err != nil {
...
func (s *SQLite3Store) createOrUpdateEthereumBalance(ctx context.Context, tx *sql.Tx, safe *Safe, balance *big.Int, assetId, safeAssetId, assetAddress string) error {
	existed, err := s.checkExistence(ctx, tx, "SELECT balance FROM ethereum_balances WHERE address=? AND asset_id=?", safe.Address, assetId)
	if err != nil {
		return err
	} else if !existed {
		cols := []string{"address", "asset_id", "asset_address", "safe_asset_id", "balance", "latest_tx_hash", "updated_at"}
		vals := []any{safe.Address, assetId, assetAddress, safeAssetId, balance.String(), "", time.Now().UTC()}
		err = s.execOne(ctx, tx, buildInsertionSQL("ethereum_balances", cols), vals...)
		if err != nil {
			return fmt.Errorf("INSERT ethereum_balances %v", err)
		}
	} else {
		err = s.execOne(ctx, tx, "UPDATE ethereum_balances SET balance=?, updated_at=? WHERE address=? AND asset_id=? AND safe_asset_id=?", balance.String(), time.Now().UTC(), safe.Address, assetId, safeAssetId)
		if err != nil {
			return fmt.Errorf("UPDATE ethereum_balances %v", err)
		}
	}
