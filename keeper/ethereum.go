...
func (node *Node) processEthereumSafeProposeTransaction(ctx context.Context, req *common.Request) error {
	...
	t, err := ethereum.CreateTransaction(ctx, ethereum.TypeETHTx, chainId, req.Id, safe.Address, outputs[0].Destination, "", outputs[0].Wei.String(), big.NewInt(outputs[0].Nonce))
