...
func (node *Node) sendTransactionUntilSufficient(ctx context.Context, assetId string, receivers []string, threshold int, amount decimal.Decimal, memo, traceId string, references []crypto.Hash) error {
	...
	_, err := common.SendTransactionUntilSufficient(ctx, node.wallet, node.mixin, []string{node.conf.App.AppId}, 1, receivers, threshold, amount, traceId, assetId, memo, references, node.conf.App.SpendPrivateKey)
	return err
}
