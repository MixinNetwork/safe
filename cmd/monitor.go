...
func bundleSignerState(ctx context.Context, mdb *mtg.SQLite3Store, store *signer.SQLite3Store, conf *signer.Configuration, grp *mtg.Group, startedAt time.Time) (string, error) {
	...
	ol, err := mdb.ListOutputsForAsset(ctx, conf.AppId, conf.KeeperAssetId, math.MaxInt64, mixin.UTXOStateUnspent, 10)
	if err != nil {
		return "", err
	}
...
func bundleKeeperState(ctx context.Context, mdb *mtg.SQLite3Store, store *kstore.SQLite3Store, conf *keeper.Configuration, grp *mtg.Group, startedAt time.Time) (string, error) {
	...
	ol, err := mdb.ListOutputsForAsset(ctx, conf.AppId, mtg.StorageAssetId, math.MaxInt64, mixin.UTXOStateUnspent, 10)
	if err != nil {
		return "", err
	}
	state = state + fmt.Sprintf("💍 XIN Outputs: %d\n", len(ol))
	ol, err = mdb.ListOutputsForAsset(ctx, conf.AppId, conf.AssetId, math.MaxInt64, mixin.UTXOStateUnspent, 10)
	if err != nil {
		return "", err
	}
	state = state + fmt.Sprintf("💍 MSKT Outputs: %d\n", len(ol))
