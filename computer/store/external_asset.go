...
var externalAssetCols = []string{"asset_id", "uri", "icon_url", "created_at", "requested_at"}

func externalAssetFromRow(row Row) (*ExternalAsset, error) {
	var a ExternalAsset
	err := row.Scan(&a.AssetId, &a.Uri, &a.IconUrl, &a.CreatedAt, &a.RequestedAt)
...
func (s *SQLite3Store) WriteExternalAssets(ctx context.Context, assets []*ExternalAsset) error {
	...
	for _, asset := range assets {
		vals := []any{asset.AssetId, nil, nil, asset.CreatedAt, nil}
